...
import com.github.imdmk.spenttime.user.UserFeature;
...
public final class SpentTimePlugin extends JavaPlugin {
     ...
     *
     * <p>The module manager resolves ordering automatically based on declared
     * interdependencies between modules.</p>
     */
     ...
     *
     * <p>The executor uses a named daemon thread ({@code SpentTime-Worker}) and logs
     * uncaught exceptions to SLF4J, ensuring no silent async failures occur.</p>
     *
    ...
    private static ExecutorService newWorkerExecutor() {
        ThreadFactory factory = runnable -> {
            ...
            thread.setUncaughtExceptionHandler((t, e) ->
                    LoggerFactory.getLogger(SpentTimePlugin.class)
                            .error("Uncaught async exception in thread {}", t.getName(), e));
            return thread;
        };
