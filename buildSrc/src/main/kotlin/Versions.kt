object Versions {

    const val SPIGOT_API = "26.2-R0.1-SNAPSHOT"

    const val PLACEHOLDER_API = "2.12.3"

    const val PANDA_DI = "1.8.0"
    const val CLASSGRAPH = "4.8.194"

    const val KYORI_PLATFORM_BUKKIT = "4.4.1"
    const val KYORI_TEXT_MINIMESSAGE = "4.26.1"

    const val MULTIFICATION_BUKKIT = "1.2.4"
    const val MULTIFICATION_OKAERI = "1.2.4"

    const val CAFFEINE = "3.2.4"
    const val HIKARI_CP = "7.1.0"
    const val ORMLITE = "6.1"

    const val OKAERI_SNAKEYAML = "5.0.13"
    const val OKAERI_SERDES_COMMONS = "5.0.13"

    const val TRIUMPH_GUI = "3.1.13"
    const val BSTATS_BUKKIT = "3.2.1"
    const val LITECOMMANDS = "3.11.0"

    const val LIBBY_BUKKIT = "2.0.0-SNAPSHOT"

    const val MYSQL_CONNECTOR_J = "26.7.0"
    const val MARIADB_CONNECTOR_J = "3.5.10"
    const val SQLITE_JDBC = "3.53.2.1"
    const val POSTGRESQL_JDBC = "42.7.13"
    const val H2_JDBC = "2.4.240"
    const val MSSQL_JDBC = "13.4.0.jre11"
}
